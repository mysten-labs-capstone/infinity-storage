...
function createDemoDownloadResponse(
  req: Request,
  blobId: string,
  filename: string,
): Response {
  const content = [
    ...
    "",
    "This is synthetic demo content generated by the server.",
  ].join("\n");
  ...
  try {
    ...
    if (shareId) {
      try {
        ...
        // Links are treated as non-expiring; clear legacy expiration when encountered.
    ...
    if (blobId.startsWith("demo_")) {
      return createDemoDownloadResponse(req, blobId, downloadName);
    }
